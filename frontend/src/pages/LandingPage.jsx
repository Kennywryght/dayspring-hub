...
import {
  ...
  GraduationCap, Users, Heart, Shield, Zap, CheckCircle2, X,
} from 'lucide-react';
...
export default function LandingPage() {
  ...
  const handleNavigate = (path) => { playClick(); navigate(path); };

  const featureItems = [
    { Icon: BookOpen, title: 'Learning Materials', desc: 'Teachers upload PDFs, videos, and notes accessible anytime.', color: 'from-navy-600 to-navy-800' },
    { Icon: ClipboardList, title: 'Assignments', desc: 'Students submit work digitally with streamlined review.', color: 'from-navy-600 to-navy-800' },
    { Icon: Megaphone, title: 'Announcements', desc: 'Important school updates delivered in real time.', color: 'from-navy-600 to-navy-800' },
  ];

  const roles = [
    { icon: '👩‍🏫', label: 'Teachers', desc: 'Manage classes, materials & assignments', Icon: Users },
    { icon: '🎓', label: 'Students', desc: 'Access learning materials & submit work', Icon: GraduationCap },
    { icon: '👨‍👩‍👧', label: 'Parents', desc: 'Monitor your child\'s progress', Icon: Heart },
    { icon: '⚙️', label: 'Admins', desc: 'System management & configuration', Icon: Shield },
  ];

  return (
    <div className="flex flex-col min-h-screen bg-white dark:bg-navy-900 overflow-hidden relative transition-colors duration-300">
      {/* Install App Prompt */}
      {showInstallPrompt && (
        <div className="fixed bottom-4 left-2 right-2 sm:bottom-6 sm:left-auto sm:right-auto sm:left-1/2 sm:-translate-x-1/2 z-50 animate-slide-up sm:max-w-md">
          <div className="bg-white dark:bg-navy-800 rounded-2xl shadow-elevated border border-ink-200 dark:border-navy-600 p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-brass-600 flex items-center justify-center text-white text-xl flex-shrink-0 shadow-lg">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        {/* Animated blobs */}
        <div className="absolute top-0 left-0 w-48 h-48 bg-navy-200 dark:bg-navy-700 rounded-full blur-3xl opacity-30 dark:opacity-20 animate-float-left" />
        ...
        <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/70 dark:bg-navy-900/70 border-b border-ink-200 dark:border-navy-700 animate-slide-down">
          <nav className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
            <div className="flex items-center gap-2 cursor-pointer">
              <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-brass-200 dark:border-brass-500/30 shadow-md flex-shrink-0">
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <button onClick={() => handleNavigate('/login')} className="bg-brass-600 hover:bg-brass-700 text-white px-5 py-2.5 rounded-2xl font-semibold text-sm shadow-soft transition-all duration-300">
                Sign In
              </button>
            </div>
            ...
            <button className="md:hidden p-2 rounded-xl hover:bg-ink-100 dark:hover:bg-navy-700 transition-colors" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
              {mobileMenuOpen ? <X className="w-6 h-6 text-navy-600 dark:text-ink-300" strokeWidth={1.75} /> : <svg className="w-6 h-6 text-navy-600 dark:text-ink-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>}
            </button>
          </nav>
          <div className={`md:hidden bg-white dark:bg-navy-900 border-b border-ink-200 dark:border-navy-700 px-4 pb-4 transition-all duration-300 ${mobileMenuOpen ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
            ...
            <button onClick={() => { handleNavigate('/login'); setMobileMenuOpen(false); }} className="w-full mt-2 bg-brass-600 text-white py-3 rounded-2xl font-semibold shadow-soft text-sm">Sign In</button>
          </div>
        </header>
        ...
        <main className="flex-grow relative">
          ...
          <section className="max-w-7xl mx-auto px-4 pt-8 pb-12 sm:pt-16 sm:pb-16 md:pt-24 md:pb-20">
            <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
              <div className="animate-fade-in-left text-center sm:text-left">
                ...
                <div className="mt-6 sm:mt-8 md:mt-10">
                  <div className="flex flex-col sm:flex-row gap-3 w-full">
                    <button onClick={() => handleNavigate('/login')} className="w-full sm:w-auto bg-brass-600 hover:bg-brass-700 text-white px-8 py-4 rounded-2xl text-base font-semibold shadow-soft transition-all duration-300 active:scale-95 text-center">
                ...
                <div className="flex justify-center sm:justify-start flex-wrap gap-6 mt-8 sm:mt-12">
                  {[{ value: '24/7', label: 'Access Anywhere' },{ value: '100%', label: 'Digital Learning' },{ value: '4 Roles', label: 'Unified Platform' }].map((stat, i) => (
              ...
              <div className="animate-fade-in-right mt-8 lg:mt-0">
                <div className="bg-navy-700 rounded-2xl sm:rounded-[2rem] p-4 sm:p-6 md:p-8 shadow-elevated">
                  <div className="bg-white dark:bg-navy-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-card">
                    <div className="space-y-3 sm:space-y-5">
                      {[{ Icon: BookOpen, title: 'Learning Materials', desc: 'Upload notes, PDFs & resources', bg: 'bg-navy-50 dark:bg-navy-700/50' },{ Icon: ClipboardList, title: 'Assignment Tracking', desc: 'Submit and review work easily', bg: 'bg-navy-50 dark:bg-navy-700/50' },{ Icon: Megaphone, title: 'Real-Time Updates', desc: 'School announcements instantly', bg: 'bg-navy-50 dark:bg-navy-700/50' }].map((item, idx) => (
                        <div key={idx} className={`flex items-center gap-3 p-3 rounded-xl sm:rounded-2xl ${item.bg} animate-fade-in-up`} style={{ animationDelay: `${idx * 0.15}s` }}>
                          <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-brass-600 flex items-center justify-center text-white text-lg sm:text-2xl flex-shrink-0"><item.Icon className="w-5 h-5 sm:w-7 sm:h-7" strokeWidth={1.75} /></div>
          ...
          <section className="max-w-7xl mx-auto px-4 pb-16 sm:pb-20">
            <div className="text-center mb-10 sm:mb-16">
              ...
              <p className="mt-3 sm:mt-4 text-sm sm:text-base text-ink-500 dark:text-ink-400 max-w-2xl mx-auto">Four dedicated portals designed for each role in the school ecosystem.</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
              {roles.map((role, i) => (
                <div key={i} className="bg-white dark:bg-navy-800 rounded-2xl shadow-card border border-ink-200 dark:border-navy-600 p-5 sm:p-6 text-center hover:shadow-elevated transition-shadow duration-300 animate-fade-in-up" style={{ animationDelay: `${i * 0.1}s` }}>
                  <div className="w-14 h-14 rounded-2xl bg-navy-700 flex items-center justify-center mx-auto mb-4"><span className="text-2xl">{role.icon}</span></div>
          ...
          <section ref={featuresRef} id="features" className="max-w-7xl mx-auto px-4 pb-16 sm:pb-20 md:pb-24">
            <div className={`text-center mb-10 sm:mb-16 ${featuresVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
              ...
              <p className="mt-3 sm:mt-4 text-sm sm:text-base text-ink-500 dark:text-ink-400 max-w-2xl mx-auto">Designed to simplify communication, learning, and school management.</p>
            </div>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
              {featureItems.map((feature, i) => (
                <div key={i} className={`group bg-white dark:bg-navy-800 border border-ink-200 dark:border-navy-600 rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 shadow-card transition-all duration-300 hover:shadow-elevated hover:-translate-y-2 cursor-pointer ${featuresVisible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: `${i * 0.2}s` }}>
                  <div className="w-14 h-14 rounded-2xl bg-navy-700 flex items-center justify-center mb-4 sm:mb-6 transform transition-transform group-hover:scale-110 duration-300">
                    <feature.Icon className="w-7 h-7 text-brass-400" strokeWidth={1.75} />
                  </div>
        ...
        <footer className="border-t border-ink-200 dark:border-navy-700 bg-ink-50/80 dark:bg-navy-800/50 backdrop-blur-sm py-6 sm:py-8 text-center">
